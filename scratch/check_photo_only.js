...
async function runTest(username, password) {
  ...
  console.log("Checking Upload Photo Page...");
  const photoRes = await client.get(`${UUCMS_BASE_URL}/Login/UploadPhotoandSignature`);
  const $photo = cheerio.load(photoRes.data);
  
  const photoImg = $photo('#imgPhoto');
  const signImg = $photo('#imgSignature');
  
  console.log(`Photo element found: ${photoImg.length}`);
  if (photoImg.length > 0) {
    console.log(`Photo src: ${photoImg.attr('src')}`);
  }
  
  // Try to find the detailed application link here too
  $photo('a').each((_, el) => {
    const href = $photo(el).attr('href') || '';
    if (href.includes('Preview')) console.log(`Found link: ${href}`);
  });
}

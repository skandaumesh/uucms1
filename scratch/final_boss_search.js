...
async function runTest(username, password) {
  ...
  console.log("Checking Course Registration...");
  const res = await client.get(`${UUCMS_BASE_URL}/CourseRegistrationByStudent/StudentCourses`);
  const $ = cheerio.load(res.data);
  
  console.log("Photo element in CourseReg:", $('img#imgPhoto, .profile-img img').length);
  
  $('a').each((_, el) => {
    const href = $(el).attr('href') || '';
    if (href.includes('Preview')) console.log(`Found link in CourseReg: ${href}`);
  });

  // FINAL BOSS: Scan the entire OnLoginSuccess for ANY encoded Admission link
  const successRes = await client.get(`${UUCMS_BASE_URL}/Login/OnLoginSucess`);
  const matches = successRes.data.match(/\/MIS\/PreviewStudentApplicationDetails\?enc=[A-Za-z0-9+/=]+/g);
  if (matches) {
     console.log("MATCHES FOUND IN SUCCESS HTML:", matches);
  } else {
     console.log("No regex matches in Success HTML.");
  }
}

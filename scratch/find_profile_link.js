...
async function runTest(username, password) {
  ...
  console.log("Logged in. Searching for profile links...");

  const pages = [
    '/Login/OnLoginSucess',
    '/StudentProfile/StudentProfile',
    '/Admission/StudentAdmission',
    '/Admission/ManageApplications'
  ];
  ...
  for (const path of pages) {
    try {
      ...
      $('a').each((_, el) => {
        ...
        if (href.includes('PreviewStudentApplicationDetails') || href.includes('Application') || text.includes('Application')) {
          links.push({ text, href });
        }
      });
      
      if (links.length > 0) {
        console.log(`  Found links in ${path}:`);
        links.forEach(l => console.log(`    - [${l.text}] -> ${l.href}`));
      }

      // Check for raw regex match in script tags or HTML
      const matches = res.data.match(/MIS\/PreviewStudentApplicationDetails\?enc=[A-Za-z0-9+/=]+/g);
      if (matches) {
        console.log(`  Found regex matches in ${path}:`);

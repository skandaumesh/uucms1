...
async function runTest(username, password) {
  ...
  console.log("Logged in. Fetching Full Profile Page HTML...");
  const res = await client.get(`${UUCMS_BASE_URL}/StudentProfile/StudentProfile`);
  fs.writeFileSync('scratch/profile_full.html', res.data);
  console.log("Saved to scratch/profile_full.html");

  console.log("Fetching Transaction History HTML...");
  const res2 = await client.get(`${UUCMS_BASE_URL}/MIS/CandidateAdmissionTransactionHistory`);
  fs.writeFileSync('scratch/history_full.html', res2.data);
  console.log("Saved to scratch/history_full.html");
}

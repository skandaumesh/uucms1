...
async function runTest(username, password) {
  ...
  console.log("Logged in. Navigating to Admission Payment History...");
  ...
  console.log("\nSEARCHING FOR PREVIEW LINKS IN HISTORY PAGE:");
  $('a').each((_, el) => {
  ...
  // Try to find ANY link in the main table
  $('table a').each((_, el) => {
     console.log(`Table Link: [${$(el).text().trim()}] -> ${$(el).attr('href')}`);
  });
}

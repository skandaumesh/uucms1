...
async function runTest(username, password) {
  ...
  console.log("Checking Admission Updates View...");
  const res = await client.get(`${UUCMS_BASE_URL}/Admission/StudentAdmissionStatusUpdatesView`);
  ...
  // Check for any "View" or "Details" buttons
  $('.btn, .btn-primary').each((_, el) => {
    const text = $(el).text().trim();
    const href = $(el).attr('href') || '';

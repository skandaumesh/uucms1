...
async function runTest(username, password) {
  ...
  console.log(`Checking direct path: ${attUrl}`);
  const attPage = await client.get(attUrl);
  
  const $att = cheerio.load(attPage.data);
  const rowCount = $att('table tr').length;
  console.log(`Found ${rowCount} rows in table`);
  
  if (rowCount > 1) {
    console.log('SUCCESS: Direct path works!');
  } else {
    console.log('FAILURE: Direct path returned empty table.');
  }
}

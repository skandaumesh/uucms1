...
async function runTest(username, password) {
  ...
  // Use the exact link provided by the user
  const attUrl = `https://uucms.karnataka.gov.in/StudentAttendance/AttendanceForStudents/?enc=sVqmbRqU5uvQnKG+d9WeUg==`;
  console.log(`Fetching Attendance from user-provided link: ${attUrl}`);
  ...
  $att('table').each((i, table) => {
    const text = $att(table).text().trim();
    if (text.length > 50) {
        console.log(`--- Table ${i+1} Content ---`);
        $att(table).find('tr').each((j, row) => {
            const cols = $att(row).find('td, th').map((_, el) => $att(el).text().trim()).get();
            console.log(`Row ${j}: ${cols.join(' | ')}`);
        });
    }
  });

  // Also check for scripts that might be doing AJAX
  console.log('--- Checking for AJAX scripts ---');
  $att('script').each((i, script) => {
    const content = $att(script).html();
    if (content && (content.includes('$.ajax') || content.includes('fetch'))) {
        console.log(`Script ${i} contains AJAX logic`);
    }
  });
}

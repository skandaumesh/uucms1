...
async function runTest(username, password) {
  ...
  // Initialize MIS session by visiting CandidateHistory first
  await client.get(`${UUCMS_BASE_URL}/MIS/CandidateAdmissionTransactionHistory`);
  ...
  if (html.includes('NullReferenceException')) {
     console.log("FAILED: Received Server Error page.");
     return;
  }

  console.log("\nALL IMAGES:");
  $('img').each((_, el) => {
    console.log(`- Src: ${$(el).attr('src')} | ID: ${$(el).attr('id')} | Alt: ${$(el).attr('alt')}`);
  });

  console.log("\nALL BOLD/LABELS:");
  $('b, strong, label, th').each((_, el) => {
  ...
  const fs = require('fs');
  fs.writeFileSync('scratch/preview_full.html', html);
}

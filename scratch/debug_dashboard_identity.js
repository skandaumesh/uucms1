...
async function runTest(username, password) {
  ...
  const res = await client.get(`${UUCMS_BASE_URL}/Login/OnLoginSucess`);
  const $ = cheerio.load(res.data);
  
  console.log("SEARCHING FOR PHOTO IN DASHBOARD:");
  $('img').each((_, el) => {
     const src = $(el).attr('src') || '';
     console.log(`Image found: ${src}`);
  });

  // Check for any personal details in the dashboard
  const welcomeText = $('h4').text().trim();
